  ...
  <div class="app-container">
    <el-col :span="24">
        ...
        <div slot="header" class="clearfix">
            <span class="blogheader">用户组列表</span>
        ...
        <div class="multiple-delete-container">
            ...
            type="success"
            plain
            @click="dialogFormVisible = true">添加用户组组</el-button>
    ...
    <el-dialog title="用户组用户组" :visible.sync="dialogFormVisible">
        <el-form :model="groups_form">
            <el-form-item label="用户组名称: " :label-width="formLabelWidth" style="margin-bottom: 10px;" prop="groups_name">
            <el-input v-model="groups_form.name" auto-complete="on"></el-input>
            </el-form-item>
            <el-form-item label="用户组备注: " :label-width="formLabelWidth" prop="groups_remark">
            <el-input type="textarea" :rows="2" v-model="groups_form.remark" auto-complete="on"></el-input>
            </el-form-item>
        </el-form>
        <div slot="footer" class="dialog-footer">
            <el-button @click="dialogFormVisible = false">取 消</el-button>
            <el-button type="primary" v-loading="groupsloading" @click="add_groups">确 定</el-button>
        </div>
    </el-dialog>
  </div>
  ...
  methods: {
    ...
    handle_delete(index, row) {
      this.$confirm('此操作将永久删除?', '提示', {
        ...
        var groups_index = this.groups_list.indexOf(row)
        if(groups_index !== -1){
            this.groups_list.splice(groups_index,1)
        }
        this.$store.dispatch('del_groups', row).then(res => {
    ...
    save_edit(row) {
      ...
      this.$store.dispatch('edit_groups', row).then(res => {
        ...
        if(this.multipleSelection.length < 1){
            ...
            this.$confirm('此操作将永久删除?', '提示', {
                ...
                this.multipleSelection.forEach(items => {
                    var index = this.groups_list.indexOf(items)
                    if(index !== -1){
                        this.groups_list.splice(index, 1)
                        this.$store.dispatch('del_groups', items).then(res => {
    ...
    add_groups(){
      if(this.groups_form.name === ''){
        ...
        this.$store.dispatch('add_groups', this.groups_form).then(() => {
          this.$notify({
            title: '成功',
            message: '添加用户组成功',
            type: 'success',
          ...
          let data = this.$store.getters.groups_list.map(v => {
            this.$set(v, 'name_edit', false)
            this.$set(v, 'remark_edit', false)
            this.$set(v, 'save_loading', false)
            this.$set(v, 'del_loading', false)
           ...
           data.forEach((item) => {
            if(this.groups_list.indexOf(item) === -1){
              this.groups_list.push(item)
